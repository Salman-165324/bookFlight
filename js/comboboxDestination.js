...
aria.GridCombobox.prototype.handleInputKeyDown = function (evt) {
  ...
  if (activeItem) {
    this.input.setAttribute(
      ...
      'destination-result-item-' + activeRowIndex + 'x' + activeColIndex
    );
...
aria.GridCombobox.prototype.updateResults = function () {
  ...
  if (results.length) {
    for (var row = 0; row < results.length; row++) {
      ...
      resultRow.setAttribute('id', 'destination-result-row-' + row);
      for (var col = 0; col < results[row].length; col++) {
        ...
        resultCell.setAttribute('id', 'destination-result-item-' + row + 'x' + col);
...
aria.GridCombobox.prototype.getItemAt = function (rowIndex, colIndex) {
  return document.getElementById('destination-result-item-' + rowIndex + 'x' + colIndex);
};
...
aria.GridCombobox.prototype.removeFocusCell = function (rowIndex, colIndex) {
  var row = document.getElementById('destination-result-row-' + rowIndex);
...
function searchData(searchString) {
  ...
  for (var i = 0; i < citiesAndCountry.length; i++) {
    var veggie = citiesAndCountry[i][0].toLowerCase();
    if (veggie.indexOf(searchString.toLowerCase()) === 0) {
      results.push(citiesAndCountry[i]);
    }
  }
