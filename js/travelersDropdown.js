...
const showDropdown = () => {
  ...
  document
    .getElementById("travelers-and-more")
...
const updateDisplayValue = () => {
  ...
  const cabinClass =
    document.getElementById("selected-value").textContent || "Economy";
  const displayValue = `${adults} Adult${
    adults > 1 ? "s" : ""
  }, ${children} Child${children > 1 ? "ren" : ""}, ${cabinClass}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyBtn = document.getElementById("travelers-apply-btn"); 

  // Toggle dropdown on input click
  travelersAndMore.addEventListener("click", (event) => {
    const isExpanded =
      travelersAndMore.getAttribute("aria-expanded") === "true";
    if (isExpanded) {
      hideDropdown();
    } else {
      showDropdown();
  ...
  document
    ...
    ?.addEventListener("change", updateDisplayValue);
  document
